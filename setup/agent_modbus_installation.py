#!/usr/bin/env python3
"""Script to install the pattoo modbus agent."""
...
from _pattoo_agent_modbus import configure
...
class _Install():
    ...
    def __init__(self, subparsers, width=80):
        ...
        parser = subparsers.add_parser(
            'install',
            help=textwrap.fill('Install pattoo modbus agent.', width=width)
        )
        # Add subparser
...
def main():
    ...
    """
    # Initialize key variables
    _help = 'This program is the CLI interface to configuring the modbus agent'
    template_dir = os.path.join(ROOT_DIR, 'setup/systemd/system')
    daemon_list = ['pattoo_agent_modbustcpd']
    ...
    if args.action == 'install':
        ...
        # Installs all pattoo modbus agent components
